...
with st.expander("ℹ️ - phoBERT", expanded=False):
    st.markdown(
        ...
        - PhoBERT has two versions, PhoBERTbase and PhoBERTlarge, using the same architectures of BETbase and 
        BERTlarge, respectively. 
        - PhoBERT pre-training approach is based on RoBERT which optimizes the BERT pre-training procedure for more 
        robust performance.
        - PhoBERT outperforms previous monolingual and multilingual approaches, obtaining new state-of-the-art 
        performances on four downstream Vietnamese NLP tasks of Part-of-speech tagging, Dependency parsing, 
    ...
    st.markdown(r"""
        ...
        - The Vietnamese Wikipedia corpus which is relatively small (1GB in size uncompressed) is the only data used 
        to train monolingual language models.
        - All publicly released monolingual and multilingual BERT-based language models are not aware of the 
        difference between Vietnamese syllables and word tokens. Without doing a pre-process step of Vietnamese word 
        segmentation, those models directly apply Byte-Pair encoding (BPE) methods to the syllable-level Vietnamese 
        pre-training data. Intuitively, for word-level Vietnamese NLP tasks, those models might not perform well
        
        => \bf{PhoBERT to handle previous concerns by}:
        
        - To handle the first problem, we train the first large-scale monolingual BERT-based using a 20GB word-level 
        Vietnamese corpus from Vietnamese Wikipedia corpus (∼1GB), and the second corpus (∼19GB) generated by 
        removing similar articles and duplication from a 50GB Vietnamese news corpus. 
        - To solve the second concern, employ RDRSegmenter (Vietnamese word segmentation) from VnCoreNLP to perform 
        word and sentence segmentation on the pre-training dataset, resulting in ∼145M word-segmented sentences, 
        before going through the BPE encoder.
    ...
    with col1:
        st.write('')
    with col2:
        st.image(r"https://phamdinhkhanh.github.io/assets/images/20200523_BERTModel/pic5.png", width=400)
    with col3:
        st.write()
...
with st.expander("ℹ️ - TFIDF", expanded=False):
    ...
    with col1:
        st.write('')
    with col2:
        st.image(r"https://ecm.capitalone.com/WCM/tech/tf-idf-4.png", width=400)
    with col3:
        st.write()
